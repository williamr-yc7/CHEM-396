...
\begin{definition}[Reverse Monte Carlo]
    Reverse Monte Carlo (RMC) randomly moves atoms until the model's computed $g(r)$ and structure factor match experimental data. It fits the measurement rather than minimizing energy, so different structures can give the same fit.
\end{definition}
...
\begin{definition}[King Criterion]
    A ring-counting rule: for each pair of bonds at an atom, the ring is the shortest path connecting them back through the network.
\end{definition}
...
\begin{definition}[Activation Relaxation Technique Nouveau]
    Activation Relaxation Technique nouveau (ARTn)~\cite{barkema1996} is a saddle-point search algorithm used to map the potential energy surface (PES)\footnote{Picture a landscape where height is how much energy a given atom arrangement has. Low valleys are stable arrangements, hilltops are unstable ones. To get from one stable arrangement to another, the atoms must pass through a higher-energy state in between, which is the hill.} and identify transitions between local minima.
\end{definition}
...
\end{definition}
\begin{definition}[Metropolis Acceptance Probability]
    A rule for accepting or rejecting a proposed move: always accept if it lowers the energy, otherwise accept with a probability that falls off exponentially with the energy increase,
    \begin{equation}
        P_{\text{acc}} = \min\!\left[1, \exp\!\left(-\frac{\Delta E}{k T}\right)\right],
        \label{eq:metropolis}
    \end{equation}
    where $\Delta E$ is the energy difference between the old and new configurations, $k$ is the Boltzmann constant, and $T$ is a fictitious temperature.
\end{definition}
Each ARTn cycle proposes one such event, accepted or rejected by Eq.~\ref{eq:metropolis} with $T$ set to $0.25$~eV in Zongo et al.~\cite{zongo2025}. A larger $T$ accepts more uphill moves, which lets the search escape shallow traps instead of sticking in the first minimum it finds. This is what is meant by ARTn ``capturing rare events''.
...
Angular distribution function & peaks at $109.5^\circ$, spread $10^\circ$ & matches experiment \\
...
\begin{definition}[Angular Distribution Function]
    The distribution of bond angles $\theta$ across the structure, written $P(\theta)$. In a-Si it peaks at $109.5^\circ$ with a spread near $10^\circ$.
\end{definition}
\begin{definition}[Keating Potential]
    An early empirical potential for tetrahedral semiconductors, penalizing only deviations in bond length and bond angle from their ideal values.
\end{definition}
...
\end{enumerate}
\begin{definition}[Core-year]
    One CPU core running for one year. The standard unit for reporting the cost of an HPC calculation, since it does not depend on how many cores ran in parallel.
\end{definition}
The cost is modest: 0.0615 core-years for the 216-atom model and 0.34 for the 1000-atom one.
...
\begin{definition}[Hyperparameter]
    A setting chosen before training rather than learned from the data, such as the voxel size, the number of convolutional layers, or the number of training epochs. Values are picked by trial and comparison rather than by fitting.
\end{definition}
...
\begin{definition}[Dynamic Heterogeneity]
    In a supercooled liquid, some regions rearrange quickly while others stay frozen, even though the material looks uniform. The motion is uneven in space rather than spread evenly.
\end{definition}
...
\subsubsection{OVITO and ISAACS}
\begin{definition}[OVITO]
    The Open Visualization Tool~\cite{ovito}, used here for coordination analysis and for the ``identify diamond structure'' crystallinity test.
\end{definition}
\begin{definition}[ISAACS]
    A structure-analysis package for amorphous and crystalline systems, used by Zongo et al. to compute the structure factor.
\end{definition}
...
\begin{enumerate}
    ...
    \item \textbf{Equilibrate.} Hold at 300~K for 150~ps to relax the structure and remove any residual stress.
\end{enumerate}
All production runs use the CPU build. The 216-atom cell is well below the $\sim$2000 atoms per GPU at which even the block-parallel GPU variant becomes worthwhile, so the GPU build validated in Section 3.2 was not used beyond that validation.
...
The NPT test was run at 300~K and 0~bar and expanded the box from $4325$ to $4433$~\AA$^3$, corresponding to a lattice parameter of $a=5.476$~\AA. This is $0.8\%$ above the measured value of $5.431$~\AA, an overestimate consistent with the PBE\footnote{Perdew-Burke-Ernzerhof, the generalized gradient approximation exchange-correlation functional used to generate the MTP training data.}~\cite{pbe1996} reference data used to train the potential.
...
\subsection{Running MAP}

MAP was installed to \texttt{\textasciitilde/MAP-main} with a virtual environment at \texttt{\textasciitilde/envMAP}, requiring \texttt{python/3.11} and \texttt{scipy-stack}.

Inspecting the bundled water dataset settled the data format. The files are \texttt{water.npy}, shape $(1000, 648, 3)$, and \texttt{water\_identity.npy}, shape $(1000, 648)$ of species strings. MAP is not given a voxel grid; it voxelizes internally. The a-Si equivalent is therefore just $(500, 216, 3)$ coordinates paired with a $(500, 216)$ array of \texttt{'Si'}.

The code writes checkpoints and reads data from hardcoded paths relative to the working directory, which would scatter output into the quota-limited home filesystem. Runs are executed from \texttt{/scratch} with \texttt{training\_samples} symlinked in and the \texttt{ckpts/} prefix stripped from \texttt{save\_ckpt}.

A validation run on unmodified water data was killed by the out-of-memory killer. This is host RAM rather than GPU memory. The fix was to request \texttt{--mem-per-gpu=100G}, matching the configuration used by the MAP authors, after which training ran to completion: 81 epochs at roughly $940$~s each, with the test negative log-likelihood falling from $0.0102$ to $0.0069$ and flattening past epoch 58. Generation of a single sample then ran its full 80 steps in 1~h 17~min before failing on the last line, where \texttt{np.save} writes to the relative path \texttt{samples/run\_0.npy} and the directory does not exist.

This is the same fault as the \texttt{ckpts/} prefix and points at a general property of the code: every output path is relative to the working directory and none is created if absent. The fix was to write the sample to an absolute path under \texttt{/scratch}. Since no checkpoint is written for a completed training stage, recovering the sample required repeating all 81 epochs, so a dry run at low \texttt{max\_epochs} is worth doing after any change to the output paths.
...
\toprule
Item & Current & Required for a-Si \\
\midrule
Data filenames & water & a-Si equivalents \\
\hline
Augmentation slices & Index 40, assuming $80^3$ & $\sim$34 per side at 0.5~\AA \\
\hline
\texttt{max\_epochs} & 80 & 500 \\
\bottomrule
\end{tabular}
\caption{Changes required to train MAP on a-Si.}
\label{tab:map_changes}
\end{table}

The voxel size remains undecided between 0.25 and 0.5~\AA. The larger value follows the quarter-bond-length rule, but Fig.~\ref{fig:vibrations} shows only $\sim1.3\times10^{-3}$ of atom-frames exceed 0.5~\AA, so most atoms never leave their voxel and successive training frames are near-identical. The choice will be made on training loss and sample quality.
...
August 19 & MAP validation run on the bundled water data killed by the out-of-memory killer. Diagnosed as host RAM rather than GPU memory and resolved with \texttt{--mem-per-gpu=100G}, matching the configuration used by the MAP authors. The \texttt{ckpts/} prefix was stripped from \texttt{save\_ckpt} so checkpoints write to the run directory rather than the quota-limited home filesystem. \\
September 5 & Water validation run completed. Training converged over 81 epochs, the test negative log-likelihood falling from $0.0102$ to $0.0069$ and flattening past epoch 58, at roughly $940$~s per training epoch and $80$~s per test pass. Generation of a single sample ran the full 80 steps in 1~h 17~min, then failed at the final write: \texttt{np.save} was called on the relative path \texttt{samples/run\_0.npy} and no such directory existed. The same class of fault as the \texttt{ckpts/} prefix, and the second time MAP's hardcoded relative output paths have cost a run. \\
September 14 & Save path in \texttt{utils.py} redirected to \texttt{/scratch}, and the water run resubmitted in full. No checkpoint was written for the completed training, so all 81 epochs are repeated to reach the generation stage. \\
\end{longtable}
