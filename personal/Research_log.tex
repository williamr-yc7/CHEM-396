...
Splitting the non-fourfold atoms by coordination shows that overcoordination
dominates in every run. Floating bonds outnumber dangling bonds three to one in
\texttt{22.almtp} and two to one in \texttt{pruned\_3}, while \texttt{pruned\_1}
has no threefold atoms at all and is the only run to produce sixfold ones.
...
\begin{table}[H]
    ...
    \begin{tabular}{l|c|c|c|c|c}
        ...
        \bottomrule
    \end{tabular}
    \caption{Coordination breakdown of the quenched solid at 300~K, using a
    2.85~\AA{} bond cutoff over the last 50~ps of each equilibration. Entries
    below $0.1\%$ are omitted. \texttt{pruned\_1} is both the densest run
    (Table~\ref{tab:quench_structural}) and the most overcoordinated, consistent
    with closer packing forcing additional neighbours inside the cutoff.}
    \label{tab:defect_breakdown}
\end{table}

\paragraph{Correlation between defects and large excursions.}
Coordination and displacement were recomputed as matched (frame, atom) arrays so
each atom's neighbour count could be read at the moment of its own displacement.
For \texttt{22.almtp}, 146 of 108{,}000 sampled points lie beyond the $0.5$~\AA{}
voxel size, and $96.6\%$ of those are non-fourfold against a $3.7\%$ background.
The effect is carried entirely by the undercoordinated atoms, which is the
expected asymmetry: a dangling bond is missing a neighbour and is correspondingly
less constrained. The tail is not a population effect, however. It comes from
nine atoms, two of which contribute 128 of the 146 points.
\begin{table}[H]
    \centering
    \small
    \begin{tabular}{l|c|c|c}
        \toprule
        & Overall (\%) & In tail (\%) & Enrichment \\
        \midrule
        ...
        \bottomrule
    \end{tabular}
    \caption{Coordination of the atoms in the $>0.5$~\AA{} tail of the
    displacement distribution for \texttt{22.almtp}, over the last 50~ps at
    300~K with a 2.85~\AA{} cutoff. The tail holds 146 of 108{,}000 samples, so
    the percentages carry counting noise.}
