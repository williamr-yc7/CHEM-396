...
\bottomrule
\end{tabular}
\caption{Signatures distinguishing the three phases of silicon. The density row is not an independent structural metric but follows from the coordination number, and provides a thermodynamic cross-check available directly from the NPT thermodynamic output: melting must contract the box rather than expand it.}
...
\begin{itemize}
    \item \textbf{RDF.} Over the last 3~ps, a single broad peak at 2.45~\AA{} damping to $g(r)=1$ by 3.5~\AA. The first 3~ps show the diamond lattice instead: peaks at 2.37, 3.87, 4.54~\AA{} persisting to 8~\AA{} with minima near 0.15.
    \item \textbf{Coordination number.} Counted within a 3.15~\AA{} cutoff, the first minimum of the liquid $g(r)$. The first 3~ps give exactly 4.000 neighbours per atom with 100\% fourfold coordination, as the diamond lattice requires. The final 3~ps give a mean of 5.968, distributed over 3- to 8-fold environments and peaking at sixfold, the denser packing expected once directional covalent bonding is lost.
...
\begin{itemize}
    ...
    \item \textbf{Density.} The box volume averages XXX~\AA$^3$ over the solid portion of the ramp and $4326$~\AA$^3$ after the transition, giving
    \begin{equation*}
        \rho = \frac{216 \times 28.0855}{6.022 \times 10^{23} \times 4326 \times 10^{-24}} = 2.329~\text{g/cm}^3
    \end{equation*}
    against XXX~g/cm$^3$ for the solid, a contraction of XXX\%. This matches the $2.337$~g/cm$^3$ obtained with \texttt{22.almtp} to within $0.4\%$.
\end{itemize}
...
XXX Do the two melt-stable potentials produce the same liquid? Compare the five numbers directly and state whether the differences are within the frame-to-frame scatter.
...
\marginpar{July 31, 2026}
 
The benchmark timings of Section~\ref{tab:melt_hold_bench} establish that all five potentials complete a nanosecond in the liquid without instability, but they are structure-agnostic: matching wall times, rebuild counts and potential energies do not establish that the potentials produce the same liquid. This section applies the five diagnostics of Section~\ref{sec:metrics} to the hold trajectories directly.
 
Unlike the melt, the hold is isothermal throughout, so there is no crystal-to-liquid contrast within a single run. The comparison is instead made across potentials, with \texttt{22.almtp} serving as the reference. All quantities are averaged over the final XXX~ps of each trajectory, by which point the liquid is fully equilibrated.
...
\toprule
Potential & $r_1$ (\AA) & $r_2$ (\AA) & $\bar{n}$ & $q$ & $D$ ($10^{-9}$~m$^2$/s) & $\rho$ (g/cm$^3$) \\
\midrule
\texttt{22.almtp}        & XXX & XXX & XXX & XXX & XXX & XXX \\
\texttt{26.almtp}        & XXX & XXX & XXX & XXX & XXX & XXX \\
\texttt{pruned\_1.almtp} & XXX & XXX & XXX & XXX & XXX & XXX \\
\texttt{pruned\_2.almtp} & XXX & XXX & XXX & XXX & XXX & XXX \\
\texttt{pruned\_3.almtp} & XXX & XXX & XXX & XXX & XXX & XXX \\
\bottomrule
\end{tabular}
\caption{Structural and dynamical properties of the liquid at 2500~K for each potential, averaged over the final XXX~ps of each hold. $r_1$ and $r_2$ are the first and second peaks of $g(r)$, $\bar{n}$ the mean coordination number within a XXX~\AA{} cutoff, $q$ the mean tetrahedral order parameter, $D$ the self-diffusion coefficient from the Einstein relation, and $\rho$ the density from the mean box volume.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \label{fig:hold_top}
    \end{subfigure}
    \caption{XXX All five hold trajectories overlaid at 2500~K. XXX}
    \label{fig:hold_comparison}
\end{figure}
 
\paragraph{Do the pruned potentials reproduce the reference liquid?}
XXX The central question. State whether the curves overlay within the frame-to-frame scatter, and if any potential deviates, in which quantity and by how much. Note that $g(r)$ and the coordination number are the sensitive tests here, since the timings and energies were already shown to agree.
 
\paragraph{Comparison with published liquid silicon.}
XXX Compare $r_1$, $\bar{n}$ and $q$ against Zhao et al.~\cite{zhao2017}, whose \textit{ab initio} molecular dynamics of liquid silicon used the same 216-atom system size. Note the temperature at which their values were obtained and whether the comparison is like for like.
 
\paragraph{Implication for the quench.}
XXX If the pruned potentials reproduce the reference liquid, the hand-off in the melt-quench protocol is justified and the cost saving is available at no structural penalty. If they do not, state which stage the deviation would affect.
