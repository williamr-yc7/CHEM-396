...
\begin{itemize}
    ...
    \begin{equation}
        ...
        \label{eq:einstein}
    \end{equation}
    where the factor 6 is $2d$ for $d = 3$ dimensions. For liquid silicon near its melting point $D$ is of order $10^{-8}$~m$^2$/s, corresponding to an MSD growing by a few \AA$^2$ per picosecond.
\end{itemize}
...
\begin{remark}
    The MSD must be computed from unwrapped coordinates. If atoms that cross a periodic boundary are folded back into the box, their apparent displacement resets and the MSD is silently corrupted. LAMMPS handles this internally in \texttt{compute msd}, which tracks image flags; a trajectory analysed externally must be dumped with unwrapped coordinates (\texttt{xu yu zu}) or with the image flags (\texttt{ix iy iz}) needed to reconstruct them. Under NPT there is a further subtlety, since box rescaling contributes an apparent displacement that is not diffusion.
\end{remark}

\subsubsection{Bond-Orientational Order Parameters}
\label{BOO}
\begin{definition}[Steinhardt Bond-Orientational Order Parameters]
    The Steinhardt parameters $q_\ell$ quantify the rotational symmetry of an atom's local neighbourhood by expanding the directions to its neighbours in spherical harmonics. For atom $i$ with $N_b(i)$ neighbours,
    \begin{equation}
        q_{\ell m}(i) = \frac{1}{N_b(i)} \sum_{j=1}^{N_b(i)} Y_{\ell m}\!\left(\hat{\mathbf{r}}_{ij}\right),
        \qquad
        q_{\ell}(i) = \sqrt{ \frac{4\pi}{2\ell + 1} \sum_{m = -\ell}^{\ell} \left| q_{\ell m}(i) \right|^{2} },
        \label{eq:steinhardt}
    \end{equation}
    where $Y_{\ell m}$ are the spherical harmonics and $\hat{\mathbf{r}}_{ij}$ is the unit vector from atom $i$ to neighbour $j$. The outer sum over $m$ makes $q_\ell$ invariant under rotation, so it depends on the shape of the neighbourhood and not on its orientation.
\end{definition}

Each crystal structure has a characteristic signature in $(q_4, q_6)$. The ideal diamond-cubic arrangement of c-Si gives $q_4 = 0.509$ and $q_6 = 0.628$, well separated from the values for other lattices. In a liquid the neighbour directions are close to random, the spherical harmonic contributions cancel, and both parameters collapse toward zero with a broad distribution and no sharp peak. a-Si sits between the two: the tetrahedral local geometry survives, so the distribution retains weight near the diamond values, but it is broadened by the spread in bond angles and shows no long-range correlation between neighbourhoods.

These are the same parameters used by Madanchi and Simine to verify that MAP-generated structures reproduce the local symmetry statistics of their reference trajectories.
...
\midrule
$g(r)$ & Sharp peaks persisting to large $r$ & Broad first peak, weak second, flat beyond & Sharp first and second peaks, no third \\
\hline
First peak position & 2.35~\AA & $\approx 2.45$~\AA & 2.35~\AA \\
\hline
Coordination number & 4 exactly & $\approx 6$ & $\approx 4$, defects below a few \% \\
\hline
MSD & Plateaus, order $0.1$~\AA$^2$ & Linear and unbounded & Plateaus \\
\hline
$(q_4, q_6)$ & $(0.509,\, 0.628)$, sharp & Near zero, broad & Broadened around tetrahedral values \\
\hline
Density & $2.33$~g/cm$^3$ & $2.57$~g/cm$^3$ & $2.28$~g/cm$^3$ \\
\bottomrule
\end{tabular}
\caption{Signatures distinguishing the three phases of silicon. Melting is confirmed by the simultaneous appearance of all four liquid signatures, not by any one of them alone.}
\label{tab:phase_signatures}
\end{table}

\begin{remark}
    These metrics are only meaningful when averaged over a trajectory at a single, equilibrated thermodynamic state. Averaging $g(r)$ across a heating ramp mixes crystalline and liquid configurations into one curve that describes neither. The structural analysis in this project is therefore performed on the isothermal hold runs rather than on the melt ramps.
\end{remark}
