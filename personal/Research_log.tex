...
Every row is a phase signature in its own right. In the crystal, $g(r)$ shows sharp peaks at the diamond-lattice neighbour shells that stay just as sharp all the way out to the 8~\AA{} box limit, with $g(r)$ falling to zero in the gaps between them: an atom at 3~\AA{} from another simply does not occur. That is what long-range order looks like. In the liquid all of it is gone but the first shell. A single broad peak sits at 2.45~\AA, and past about 3.5~\AA{} the curve flattens onto $g(r) = 1$, meaning that beyond one bond length an atom is as likely to be found anywhere as anywhere else.
...
The last two rows agree. The tetrahedral order parameter loses half its value and its distribution broadens by a factor of 35, the local-geometry counterpart of the loss of order in $g(r)$. The box contracts by $4.2\%$ and $4.0\%$ respectively, which is the correct sign for silicon: unlike most materials, liquid silicon is denser than the crystal, so melting must shrink the cell rather than expand it.
...
\begin{figure}[H]
    ...
    \caption{The 216-atom system during the \texttt{22.almtp} melt, visualized in VMD. The ordered diamond lattice visibly breaks apart into a disordered liquid over the 30~ps ramp.}
    \label{fig:melt_vmd}
\end{figure}

\paragraph{The two potentials produce the same liquid.}
Every quantity in Table~\ref{tab:melt_signatures} agrees to better than $1.5\%$: the first peak of $g(r)$ to under one histogram bin, mean coordination to $0.07\%$, density to $0.34\%$, and the melting transition itself to one dump frame (25.6 against 25.7~ps). Rebuild counts, 400 against 374, are likewise indistinguishable. None of these differences is resolvable given the scatter of a 216-atom cell. Level 26 costs $2.5\times$ more for the same result, so it was not carried into the quench.
\begin{table}[H]
    \centering
    \small
    \begin{tabular}{l|c|r|r|r|r}
        ...
        \bottomrule
    \end{tabular}
    \caption{Melt runs, 30~ps on 8 MPI tasks. No dangerous builds in either.}
...
Each melted configuration was held at 2500~K for 1~ns, continuing directly from the saved state via \texttt{read\_data} rather than rebuilding a lattice, with \texttt{fix npt} holding start and target temperature equal.
...
\begin{table}[H]
    ...
    \begin{tabular}{l|l|c|r|r|r|r}
        ...
        \bottomrule
    \end{tabular}
    \caption{Hold runs at 2500~K on 8 MPI tasks ($\geq 98.7\%$ CPU use throughout). The \texttt{22.almtp} and pruned holds start from the \texttt{22.almtp} melt; the \texttt{26.almtp} hold starts from its own melt and ran 0.4~ns, so its throughput rather than its wall time is the comparable quantity. No dangerous builds in any run.}
...
A faster run does not by itself mean a cheaper potential. If one potential let the atoms move less, LAMMPS would rebuild neighbour lists fewer times and the run would finish sooner for reasons having nothing to do with the potential. That is not what happened here: rebuild counts and neighbour numbers are the same across all five, so the trajectories behaved identically and the speedup is a genuine reduction in the cost of computing a force.
...
The measured level-22 hold took 10.6~h against the 9.1~h projected in Table~\ref{tab:projected_cost}, $16\%$ over. That gap is the 30{,}215 neighbour-list rebuilds, which the static 300~K benchmark never incurred, as anticipated.

\subsubsection{Structural Comparison}

Matching wall times, rebuild counts and potential energies are structure-agnostic and do not establish that the potentials produce the same liquid. Since the hold is isothermal there is no crystal-to-liquid contrast within a single run, so the comparison is made across potentials with \texttt{22.almtp} as the reference, over the final 30~ps of each trajectory.
...
All five potentials give the same liquid. The first peak of $g(r)$ falls at 2.44~\AA{} for every one and the curves overlay within their linewidth. Mean coordination runs from 5.912 to 5.970 across the five, a spread of $1.0\%$, and the tetrahedral order parameter from 0.4821 to 0.4975, a spread of $3.1\%$.
...
No like-for-like published comparison exists. Zhao et al.~\cite{zhao2017} report $q$ for liquid silicon, but between 1032 and 1800~K, where it runs from 0.52 to 0.68 and rises as the liquid cools. This hold is at 2500~K, $800$~K above the melting point, and gives $0.490$. Lower than their coldest value, which is the right direction, since tetrahedral character is lost on heating. Trend only, not a number.

So the hand-off is justified rather than assumed: a configuration passed from the melt-stable potential to a pruned one is indistinguishable from one the pruned potential made itself, and the $11.7\times$ saving costs nothing structurally in the liquid. Whether it survives the quench is another matter, since \texttt{pruned\_1} held $-12{,}400$~bar in a solid box where \texttt{22.almtp} sat at zero (Section~\ref{sec:firstruns}).

% ===========================================================
\subsection{Quench}

Each liquid was cooled from 2500~K to 300~K at 1~K/ps under NPT at 0~bar ($2.2\times10^{6}$ steps), followed by a $1.5\times10^{5}$-step isothermal equilibration at 300~K. Each potential quenched its own hold configuration, so melt, hold and quench share a single force model within each run and the hand-off is not exercised here.
...
The reason is that computing forces is not the only thing a run does. Cores also spend time swapping atoms with each other, and that cost stays the same no matter which potential is loaded. Under \texttt{22.almtp} forces take $91.5\%$ of the run and swapping only $8.4\%$, so the swapping barely matters. Under \texttt{pruned\_1} forces are down to $81.2\%$ and swapping is up to $13.2\%$. Make the potential cheaper still and the swapping cost is all that is left, which caps the total gain near $5\times$ however cheap the potential gets.

The four potentials agree on energy but not on density. Potential energies match to $0.015$~eV/atom, comfortably inside $k_BT = 0.026$~eV at 300~K. Density splits them: \texttt{pruned\_2} lands within $0.05\%$ of \texttt{22.almtp}, \texttt{pruned\_3} sits $0.5\%$ above, and \texttt{pruned\_1} $1.7\%$ above.

The \texttt{pruned\_1} result is the one predicted back in Section~\ref{sec:firstruns}. In a fixed box where \texttt{22.almtp} sat at zero pressure, \texttt{pruned\_1} held $-12{,}400$~bar, meaning it was pulling inward and wanted a smaller box. Here the box is free to move, so it takes one. Note that this flips between phases: \texttt{pruned\_1} is the least dense of the five in the 2500~K liquid and the most dense at 300~K. Its $2.288$~g/cm$^3$ is also the closest of the four to the experimental a-Si value of $2.28$, with \texttt{22.almtp} $1.4\%$ low at $2.249$.
...
The first \texttt{pruned\_3} quench attempt terminated at step $2.94\times10^{5}$ with an I/O error on the dump write (\texttt{dump.cpp:543}). The error was \texttt{EIO} rather than \texttt{EDQUOT} and scratch stood at 351~MB of 20~TB, so the cause was a transient Lustre fault rather than a quota. No restart file survived, so the run was repeated from the equilibrated liquid with the restart interval halved and the ramp dump thinned tenfold.

\subsubsection{Structural Comparison}

The quench produced amorphous silicon, not a frozen liquid and not a crystal. The clearest evidence is the second peak of $g(r)$: it survives at 3.88~\AA{} in all four runs, where the liquid had nothing beyond 3.5~\AA. The third shell of the crystal at 4.50~\AA{} is gone, replaced by a shallow trough. Keeping the second peak while losing the third is the defining signature of a continuous random network.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{The quenched solid at 300~K, over the last 50~ps of each run, except the MSD which uses the full 150~ps. The second peak of $g(r)$ at 3.88~\AA{} survives while the third is absent, which is what separates an amorphous network from a frozen liquid. The MSD is flat for all four, confirming the atoms vibrate rather than diffuse. Coordination peaks sharply at fourfold and the tetrahedral order parameter near $0.93$.}
...
Zongo et al.~\cite{zongo2025} built four 216-atom models with the same potential family, two by ARTn and two by melt-quench MD. Their ARTn models give $0.93\%$ and $0\%$ defects, their MD models $1.85\%$ and $1.86\%$. The ordering matches their central claim, that melt-quench traps defects where ARTn relaxes them out. These runs sit above both, at two to four times their MD figures. Part of that gap is method: their structures are relaxed to $0$~K, while these are averaged over 500 frames at 300~K, where an atom vibrating near the $2.85$~\AA{} cutoff flickers in and out of the count.
