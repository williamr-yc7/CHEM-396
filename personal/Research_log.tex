...
\subsubsection{Parallel Scaling and Potential Cost}
\marginpar{July 22, 2026}
The single-core timings from the previous runs made the cooling stage impractical, so the next step was to find out how much of that cost could be removed. Two things were tested: running across several CPU cores at once, and switching from the melt-stable potential to a cheaper pruned one.

\paragraph{Parallel scaling.}
The same 5000-step NVT run was repeated on 1, 4, and 8 cores of a single node. Running in parallel requires two changes to the submission script: requesting more tasks with \texttt{--ntasks}, and launching LAMMPS through \texttt{srun} rather than calling it directly, so that SLURM starts one copy of the program per task and connects them through MPI. Narval also requires memory to be requested per core (\texttt{--mem-per-cpu}) rather than as a total once more than one task is used.
...
\paragraph{Cost of the pruned potential.}
The same run was then repeated on 8 cores with \texttt{pruned\_1.almtp} in place of \texttt{22.almtp}. It completed in 14 seconds rather than 2 minutes 43 seconds, roughly $12\times$ faster. Taken together with the parallel speedup, this removes the obstacle identified previously: a cooling run that would have taken over a week on a single core with the melt-stable potential is now a matter of hours.

\paragraph{The two potentials prefer different volumes.}
An unexpected result came out of the same test. With \texttt{22.almtp}, the pressure at a box volume of $4433$~\AA$^3$ fluctuated around zero, meaning the atoms sit at a spacing that potential is content with. With \texttt{pruned\_1.almtp} in the identical box, the pressure instead held steady at about $-12{,}400$~bar. A negative pressure means the atoms are pulling inward: this potential would prefer a smaller box than the one it has been given.

This matters for the melt-quench protocol, where the potential is deliberately switched partway through. The equilibrium box size found with one potential is not the equilibrium box size for another, so the volume cannot simply be carried across the handoff. The likely correction is to repeat the NPT equilibration after switching potentials, so that the cooling stage begins from a box the pruned potential is actually content with, and then run NVT at that new size. %To change after response from Lena


\subsubsection{Choosing the Ensemble and Setting the Cooling Rate}
\marginpar{July 23, 2026}
Two questions had to be settled before the melt could be run: which thermodynamic ensemble to use, and how to impose a cooling rate in LAMMPS.

\paragraph{Ensemble.}
Zongo et al. (2025) never mention NVT: the only ensemble they name is NPT, for the final 150~ps equilibration at 300~K. Since the aim is to reproduce their protocol, NPT is used throughout. This also resolves the potential-volume mismatch found earlier on its own, since the box is free to relax to whatever volume the current potential prefers after a switch.

\paragraph{Cooling rate.}
...
\end{equation}
where $\Delta t$ is the timestep. For the paper's cooling rate of 1~K/ps from 2500~K to 300~K, the temperature drop of 2200~K implies 2200~ps of simulated time, which at a 1~fs timestep is $2.2 \times 10^6$ steps.

\paragraph{Projected cost.}
Combining this with the timings measured previously gives a concrete estimate for the full protocol. On 8 cores, \texttt{pruned\_1} runs at roughly 357 steps/s and \texttt{22.almtp} at roughly 31 steps/s.
...
\hline
Heating (300--2500~K) & 30~ps & $3.0 \times 10^4$ & $\sim$16~min \\
Hold at 2500~K & 1~ns & $1.0 \times 10^6$ & $\sim$9~h \\
Cooling (1~K/ps) & 2.2~ns & $2.2 \times 10^6$ & $\sim$1.7~h \\
\hline
\end{tabular}
\caption{Projected cost of the melt-quench protocol for 216 atoms on 8 cores, assuming the melt-stable potential for heating and the hold, and \texttt{pruned\_1} for cooling.}
\label{tab:projected_cost}
\end{table}

The hold at 2500~K dominates, despite involving less than half the steps of the cooling stage, because the melt-stable potential is roughly twelve times more expensive per step. Whether the hold genuinely requires the melt-stable potential, or whether the switch to a pruned one can happen earlier, has been raised with the group. These figures also assume the cost per step stays as measured on a static 300~K crystal; a hot liquid rearranges continuously and will trigger neighbour-list rebuilds that the earlier runs did not, so the true cost will be somewhat higher.

\paragraph{First melt run.}
The heating stage was then set up: the same 216-atom diamond lattice, with \texttt{fix npt} ramping from 300~K to 2500~K while holding the pressure at 0~bar, run for 30{,}000 steps to give the 30~ps heating time used in the paper. The thermostat damping constant was kept at $0.1$~ps, roughly a hundred timesteps, which is the usual choice; a value comparable to the length of the run itself would leave the thermostat too sluggish to follow the ramp at all.

\paragraph{Choice of potential for the hold.}
Whether the 1~ns hold at 2500~K requires the melt-stable potential or could use a cheaper pruned one was raised with the group. Since both potential types are being used here for the first time, the decision was to run the hold both ways and compare the resulting structures directly, treating the difference as a test of where the pruned potentials break down at high temperature rather than as a cost question alone.

\subsubsection{First Melt and the 2500 K Hold}
\marginpar{July 24, 2026}
The heating run from the previous day was checked and had melted successfully. The temperature reached the 2500~K target, and two signatures confirmed the crystal had become a liquid: the pressure now swung over a range of order $10^4$~bar rather than the $10^3$ of the solid, and the number of neighbour-list rebuilds rose from zero (nothing moves in a static crystal) to 400, the direct signature of atoms diffusing. Loading the trajectory into VMD showed the ordered diamond lattice visibly breaking apart into a disordered liquid over the course of the run.
...
With a melted configuration in hand, the next stage of the protocol was set up: the 1~ns hold at 2500~K. Rather than rebuilding a lattice, this run continues directly from the melted state using \texttt{read\_data} on the saved configuration, with \texttt{fix npt} now holding both the start and target temperature at 2500~K. Following the group's suggestion, the hold is being run with both the melt-stable (\texttt{22.almtp}) and a pruned potential and compared, since both are being used here for the first time and it is not yet known how the pruned potentials behave at high temperature. At the measured single-potential speeds this is roughly a nine-hour run, so it was submitted to run overnight.

A second melt using \texttt{26.almtp} was also launched in the background, to check whether the two melt-stable potentials produce the same behaviour.
